...
public class TpaCommand implements Command {
    ...
    public LiteralArgumentBuilder<CommandSource> getCommand() {
        return Commands.literal("tpa").requires(source -> {
            try {
    ...
    private int executeCommand(ServerPlayerEntity source, ServerPlayerEntity target) {
        ...
        if (!TeleportUtil.requestTeleport(source, source, target)) {
        ...
        target.sendMessage(new StringTextComponent("玩家 " + source.getDisplayName().getString() + " 想傳送到你這裡").mergeStyle(TextFormatting.YELLOW), Util.DUMMY_UUID);
